...
import {Columns, Edit, FileSpreadsheet, MoreHorizontal, Search, X} from 'lucide-react';
import * as XLSX from 'xlsx';

const CustomAgGridDataTable2 = ({
                                    title,
                                    subtitle,
                                    primaryButton,
                                    secondaryButton,
  ...
  useEffect(() => {
    if (initialColumns?.length > 0) {
      let processedColumns = initialColumns.map(col => ({ ...col, hide: false }));
      
      // Add edit column if showEditButton is true
      if (showEditButton) {
        processedColumns.push({
          ...
          lockPosition: true // Lock the column position
        });
      }
      setColumns(processedColumns);
    }
  }, [initialColumns, showEditButton, rowSelected]);
...
export default CustomAgGridDataTable2;